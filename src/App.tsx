...
const App: React.FC = () => {
  ...
  const formSchema = object({
    fullName: string("Name must be a string", [
      minLength(1, "Please enter your full name"),
    ]),
    discordId: string("Discord username must be a string", [
      minLength(1, "Please enter your discord ID"),
    ]),
    mobileNumber: string("Your mobile must be a number", [
      regex(/^\d{10}$/, "Please enter a valid mobile number"),
    ]),
    github: optional(
      string("Github must be a string", [
        url("enter a valid url"),
        startsWith("https://github.com", "Please enter a valid github url"),
      ]),
    ),
    linkedin: optional(
      string("LinkedIn must be a string", [
        url("enter a valid url"),
        startsWith(
  ...
  const submitHandler = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    Object.keys(formState).forEach((key) => {
      if (
        !["fullName", "discordId", "mobileNumber"].includes(key) &&
    ...
    if (result.issues && result.issues.length > 0) {
      ...
      API.register({
        ...
        tech: Object.keys(formState.roles).filter(
          (key) => formState.roles[key as keyof FormState["roles"]],
        ),
      })
        .then(async (res) => {
          setLoading(false);
        ...
        .catch((err) => {
          ...
          MySwal.fire({
            title: (
              <p>{err || "Something went wrong. Please try again later!"}</p>
            ),
            icon: "error",
          });
          setLoading(false);
          ...
          <form id="discordForm" onSubmit={submitHandler} autoComplete="off">
            ...
            <div className="form-item">
              <label htmlFor="discordId">
                Discord ID<span className="required">*</span>
              </label>
              <input
                className="text-field"
                type="text"
                ...
                onChange={(e) =>
                  setFormState({ ...formState, discordId: e.target.value })
                }
              />
              {formErrors.discordId && (
                <span className="error">{formErrors.discordId}</span>
              )}
            </div>
            <div className="form-item">
            ...
            <div className="form-item">
              ...
              />
              {formErrors.github && (
                <span className="error">{formErrors.github}</span>
              )}
            </div>
            <div className="form-item">
              ...
              />
              {formErrors.linkedin && (
                <span className="error">{formErrors.linkedin}</span>
              )}
            </div>
            <div className="form-item">
              ...
              <div id="roles">
                <span>
                  <input
                    ...
                    name="frontEnd"
                    id="frontEnd"
                    checked={formState.roles?.frontEnd}
                    onChange={(e) =>
                      setFormState({
                        ...formState,
                        roles: {
                ...
                <span>
                  <input
                    ...
                    name="backEnd"
                    id="backEnd"
                    checked={formState.roles?.backEnd}
                    onChange={(e) =>
                      setFormState({
                        ...formState,
                        roles: {
